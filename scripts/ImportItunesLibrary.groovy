...
target(importItunesLibrary: 'Import iTunes Library') {
	...
	if(args) {
		...
		if(file.exists()) {
			...
			def artistClass = classLoader.loadClass('com.tunes.Artist')
			def albumClass = classLoader.loadClass('com.tunes.Album')
			def songClass = classLoader.loadClass('com.tunes.Song')
				...
				artistClass.withTransaction { tx ->				
					...
					albums.each { albumTitle, albumDetails ->					
						...
						albumDetails.tracks?.each { trackInfo ->
							song = songClass.newInstance(title: trackInfo.name,
