            ...
            artists.each { artistName, albums ->
                ...
                artistClass.withTransaction { tx ->
                    ...
                    albums.each { albumTitle, albumDetails ->
                        album = albumClass.newInstance(title: albumTitle, year: albumDetails.year ?: 2012, genre: albumDetails.genre ?: 'Unknown')
                        artist.addToAlbums album
                        album.save(failOnError: true)
                        artist.save(failOnError: true)
                        albumDetails.tracks?.each { trackInfo ->
                            song = songClass.newInstance(title: trackInfo.name, genre: trackInfo.genre ?: 'Unknown', year: trackInfo.year ?: 2012, trackNumber: trackInfo.tracknumber ?: 1, duration: trackInfo.totaltime ?: 0, artist: artist)
